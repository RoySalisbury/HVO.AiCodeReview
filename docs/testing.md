...
| `DepthModeComprehensiveTests.cs` | 49 | Unit + Integration | Comprehensive depth mode coverage (Issue #33): DeriveQuickVerdict/Vote/Justification boundary tests, BuildQuickModeResult (change-type counts, skipped files, file review paths), BuildSummaryMarkdown edge cases (cross-file analysis, deep analysis sections, re-review badges), Deep mode Pass 3 failure/null fallback, verdict override to Rejected, consistent verdict no-override, Quick/Standard/Deep end-to-end validation. |
...
| `DeltaReviewTests.cs` | 35 | Unit | Incremental review (Issue #13): DeltaReviewInfo model defaults/population, BuildSummaryMarkdown delta section rendering (iteration range, file counts, token savings, collapsible file lists, empty states), ReviewResponse.DeltaInfo property, ReviewHistoryEntry delta fields, FakeDevOpsService.GetIterationChangesAsync (seeding, case-insensitivity, overwrite), token savings estimation (proportional, zero, heavy carry-forward), delta file partitioning (split, all-changed, case-insensitive matching), edge cases (empty changed files, multiple iteration skip, session ID + delta coexistence, large file lists). |
...
| `ReviewSessionStoreTests.cs` | 16 | Unit | In-memory session store: add/get round-trip, active session filtering, queued/in-progress counts, cancel-queued lifecycle, atomic state transitions, eviction, status enum validation, interface compliance. |
| `ReviewQueueServiceTests.cs` | 14 | Unit | Review queue service: enqueue capacity, queue-full rejection, worker processing, concurrent review execution, cancelled-session skipping, atomic state transition guard, orchestrator error handling, progress reporting, cancellation during processing, timeout, graceful shutdown. |
