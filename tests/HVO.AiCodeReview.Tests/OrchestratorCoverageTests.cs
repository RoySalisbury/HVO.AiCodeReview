...
public class OrchestratorCoverageTests
{
    ...
    [TestMethod]
    public async Task CompletedPr_StillReviewsNormally()
    {
        var fakeDevOps = new FakeDevOpsService();
        var pr = new PullRequestInfo
        {
        ...
        Assert.AreEqual("Reviewed", result.Status, "Completed PR should still be reviewed normally");
        Assert.IsNotNull(result.Summary, "Should produce a summary for completed PR");
        Assert.IsNotNull(result.Recommendation, "Should produce a recommendation for completed PR");
        // ADO writes should still be attempted (and succeed with fake)
        Assert.IsTrue(fakeDevOps.PostedComments(Project, Repo, PrId).Count > 0, "Should post summary comment even on completed PR");
    }

    [TestMethod]
    public async Task AbandonedPr_StillReviewsNormally()
    {
        var fakeDevOps = new FakeDevOpsService();
        var pr = new PullRequestInfo
        {
        ...
        Assert.AreEqual("Reviewed", result.Status, "Abandoned PR should still be reviewed normally");
        Assert.IsNotNull(result.Summary, "Should produce a summary for abandoned PR");
    }
