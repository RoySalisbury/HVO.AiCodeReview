...
| [Testing](docs/testing.md) | Disposable test repositories, 6-layer safety system, test categories (509 tests), test infrastructure, running tests, code coverage, test configuration, test roadmap. |
| [Model Benchmarks](docs/model-benchmarks.md) | Known-bad-code test issues, live benchmark results (2026-03-01), model comparison, depth → model mapping, benchmark history, running benchmarks. |
...
509 tests across Unit, Integration, LiveDevOps, LiveAI, Benchmark, and Manual categories. Tests run against disposable Azure DevOps repositories with a 6-layer safety system to prevent accidental deletion.
...
| Metric | Value | Target |
|--------|-------|--------|
| **Line coverage** | 50.4% (3,181 / 6,308) | ≥ 70% |
| **Branch coverage** | 42.4% (1,001 / 2,359) | ≥ 60% |
| **Method coverage** | 81.1% (436 / 537) | — |

Coverage is collected via [coverlet](https://github.com/coverlet-coverage/coverlet) with reports generated by [ReportGenerator](https://github.com/danielpalme/ReportGenerator). See [Issue #54](https://github.com/RoySalisbury/HVO.AiCodeReview/issues/54) for the coverage improvement plan.

```bash
# Run tests with code coverage
dotnet test tests/HVO.AiCodeReview.Tests \
  --settings tests/HVO.AiCodeReview.Tests/coverage.runsettings \
